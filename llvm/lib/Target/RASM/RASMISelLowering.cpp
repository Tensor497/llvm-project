//=- RASMISelLowering.cpp - RASM DAG Lowering Implementation  ---===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines the interfaces that RASM uses to lower LLVM code into
// a selection DAG.
//
//===----------------------------------------------------------------------===//
...
#include "RASM.h"
#include "RASMMachineFunctionInfo.h"
...
#include "llvm/IR/IntrinsicsRASM.h"
...
RASMTargetLowering::RASMTargetLowering(const TargetMachine &TM,
                                                 const RASMSubtarget &STI)
    : TargetLowering(TM), Subtarget(STI) {

  MVT GRLenVT = Subtarget.getGRLenVT();
  // Set up the register classes.
  addRegisterClass(GRLenVT, &RASM::GPRRegClass);
  if (Subtarget.hasBasicF())
    addRegisterClass(MVT::f32, &RASM::FPR32RegClass);
  if (Subtarget.hasBasicD())
    addRegisterClass(MVT::f64, &RASM::FPR64RegClass);
  ...
  setStackPointerRegisterToSaveRestore(RASM::R3);
...
bool RASMTargetLowering::isOffsetFoldingLegal(
...
SDValue RASMTargetLowering::LowerOperation(SDValue Op,
...
SDValue RASMTargetLowering::lowerWRITE_REGISTER(SDValue Op,
                                                     SelectionDAG &DAG) const {
...
SDValue RASMTargetLowering::lowerFRAMEADDR(SDValue Op,
                                                SelectionDAG &DAG) const {
...
SDValue RASMTargetLowering::lowerRETURNADDR(SDValue Op,
                                                 SelectionDAG &DAG) const {
...
SDValue RASMTargetLowering::lowerEH_DWARF_CFA(SDValue Op,
                                                   SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
...
SDValue RASMTargetLowering::lowerVASTART(SDValue Op,
                                              SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  auto *FuncInfo = MF.getInfo<RASMMachineFunctionInfo>();
...
SDValue RASMTargetLowering::lowerUINT_TO_FP(SDValue Op,
                                                 SelectionDAG &DAG) const {
  ...
  if (Op0->getOpcode() == RASMISD::BSTRPICK &&
...
SDValue RASMTargetLowering::lowerSINT_TO_FP(SDValue Op,
                                                 SelectionDAG &DAG) const {
  assert(Subtarget.is64Bit() && Subtarget.hasBasicF() &&
         !Subtarget.hasBasicD() && "unexpected target features");
...
SDValue RASMTargetLowering::lowerBITCAST(SDValue Op,
                                              SelectionDAG &DAG) const {
  ...
  if (Op.getValueType() == MVT::f32 && Op0.getValueType() == MVT::i32 &&
    ...
    return DAG.getNode(RASMISD::MOVGR2FR_W_LA64, DL, MVT::f32, NewOp0);
  }
  return Op;
}

SDValue RASMTargetLowering::lowerFP_TO_SINT(SDValue Op,
                                                 SelectionDAG &DAG) const {
  ...
  if (Op.getValueSizeInBits() > 32 && Subtarget.hasBasicF() &&
      !Subtarget.hasBasicD()) {
    SDValue Dst =
        DAG.getNode(RASMISD::FTINT, DL, MVT::f32, Op.getOperand(0));
    return DAG.getNode(RASMISD::MOVFR2GR_S_LA64, DL, MVT::i64, Dst);
  }

  EVT FPTy = EVT::getFloatingPointVT(Op.getValueSizeInBits());
  SDValue Trunc = DAG.getNode(RASMISD::FTINT, DL, FPTy, Op.getOperand(0));
...
SDValue RASMTargetLowering::getAddr(NodeTy *N, SelectionDAG &DAG,
                                         bool IsLocal) const {
  SDLoc DL(N);
  ...
  if (IsLocal)
    ...
    return SDValue(DAG.getMachineNode(RASM::PseudoLA_PCREL, DL, Ty, Addr),
                   0);

  // This generates the pattern (PseudoLA_GOT sym), which expands to (ld.w/d
  // (pcalau12i %got_pc_hi20(sym)) %got_pc_lo12(sym)).
  return SDValue(DAG.getMachineNode(RASM::PseudoLA_GOT, DL, Ty, Addr), 0);
}

SDValue RASMTargetLowering::lowerBlockAddress(SDValue Op,
                                                   SelectionDAG &DAG) const {
  return getAddr(cast<BlockAddressSDNode>(Op), DAG);
}

SDValue RASMTargetLowering::lowerJumpTable(SDValue Op,
                                                SelectionDAG &DAG) const {
  return getAddr(cast<JumpTableSDNode>(Op), DAG);
}

SDValue RASMTargetLowering::lowerConstantPool(SDValue Op,
                                                   SelectionDAG &DAG) const {
  return getAddr(cast<ConstantPoolSDNode>(Op), DAG);
}

SDValue RASMTargetLowering::lowerGlobalAddress(SDValue Op,
                                                    SelectionDAG &DAG) const {
...
SDValue RASMTargetLowering::getStaticTLSAddr(GlobalAddressSDNode *N,
                                                  SelectionDAG &DAG,
                                                  unsigned Opc) const {
  SDLoc DL(N);
  EVT Ty = getPointerTy(DAG.getDataLayout());
  ...
  return DAG.getNode(ISD::ADD, DL, Ty, Offset,
                     DAG.getRegister(RASM::R2, GRLenVT));
}

SDValue RASMTargetLowering::getDynamicTLSAddr(GlobalAddressSDNode *N,
                                                   SelectionDAG &DAG,
                                                   unsigned Opc) const {
  SDLoc DL(N);
  EVT Ty = getPointerTy(DAG.getDataLayout());
...
SDValue
RASMTargetLowering::lowerGlobalTLSAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  GlobalAddressSDNode *N = cast<GlobalAddressSDNode>(Op);
  assert(N->getOffset() == 0 && "unexpected offset in global node");
...
SDValue
RASMTargetLowering::lowerINTRINSIC_WO_CHAIN(SDValue Op,
                                                 SelectionDAG &DAG) const {
  ...
  case Intrinsic::thread_pointer: {
    ...
    return DAG.getRegister(RASM::R2, PtrVT);
  }
  }
}

SDValue
RASMTargetLowering::lowerINTRINSIC_W_CHAIN(SDValue Op,
                                                SelectionDAG &DAG) const {
...
SDValue RASMTargetLowering::lowerINTRINSIC_VOID(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDLoc DL(Op);
  MVT GRLenVT = Subtarget.getGRLenVT();
  SDValue Op0 = Op.getOperand(0);
  ...
  case Intrinsic::loongarch_dbar: {
    ...
    return DAG.getNode(RASMISD::DBAR, DL, MVT::Other, Op0,
  ...
  case Intrinsic::loongarch_ibar: {
    ...
    return DAG.getNode(RASMISD::IBAR, DL, MVT::Other, Op0,
                       DAG.getConstant(Imm, DL, GRLenVT));
  }
  case Intrinsic::loongarch_break: {
    ...
    return DAG.getNode(RASMISD::BREAK, DL, MVT::Other, Op0,
                       DAG.getConstant(Imm, DL, GRLenVT));
  }
  case Intrinsic::loongarch_syscall: {
    ...
    return DAG.getNode(RASMISD::SYSCALL, DL, MVT::Other, Op0,
                       DAG.getConstant(Imm, DL, GRLenVT));
  }
  }
}

SDValue RASMTargetLowering::lowerShiftLeftParts(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue RASMTargetLowering::lowerShiftRightParts(SDValue Op,
                                                      SelectionDAG &DAG,
                                                      bool IsSRA) const {
  SDLoc DL(Op);
  SDValue Lo = Op.getOperand(0);
...
static RASMISD::NodeType getRASMWOpcode(unsigned Opcode) {
  switch (Opcode) {
  default:
    llvm_unreachable("Unexpected opcode");
  case ISD::SHL:
    return RASMISD::SLL_W;
  case ISD::SRA:
    return RASMISD::SRA_W;
  case ISD::SRL:
    return RASMISD::SRL_W;
  case ISD::ROTR:
    return RASMISD::ROTR_W;
  case ISD::ROTL:
    return RASMISD::ROTL_W;
  case ISD::CTTZ:
    return RASMISD::CTZ_W;
  case ISD::CTLZ:
    return RASMISD::CLZ_W;
  }
}
...
static SDValue customLegalizeToWOp(SDNode *N, SelectionDAG &DAG, int NumOp,
  ...
  SDLoc DL(N);
  RASMISD::NodeType WOpcode = getRASMWOpcode(N->getOpcode());
...
void RASMTargetLowering::ReplaceNodeResults(
  ...
  case ISD::FP_TO_SINT: {
    ...
    if (getTypeAction(*DAG.getContext(), Src.getValueType()) !=
      ...
      SDValue Dst = DAG.getNode(RASMISD::FTINT, DL, FVT, Src);
  ...
  case ISD::BITCAST: {
    ...
    if (VT == MVT::i32 && SrcVT == MVT::f32 && Subtarget.is64Bit() &&
        Subtarget.hasBasicF()) {
      SDValue Dst =
          DAG.getNode(RASMISD::MOVFR2GR_S_LA64, DL, MVT::i64, Src);
  ...
  case ISD::BSWAP: {
    ...
    case 16:
      Tmp = DAG.getNode(RASMISD::REVB_2H, DL, GRLenVT, NewSrc);
      break;
    case 32:
      // Only LA64 will get to here due to the size mismatch between VT and
      // GRLenVT, LA32 lowering is directly defined in RASMInstrInfo.
      Tmp = DAG.getNode(RASMISD::REVB_2W, DL, GRLenVT, NewSrc);
      break;
    }
  ...
  case ISD::BITREVERSE: {
    ...
    default:
      llvm_unreachable("Unexpected operand width");
    case 8:
      Tmp = DAG.getNode(RASMISD::BITREV_4B, DL, GRLenVT, NewSrc);
      break;
    case 32:
      Tmp = DAG.getNode(RASMISD::BITREV_W, DL, GRLenVT, NewSrc);
      break;
    }
    Results.push_back(DAG.getNode(ISD::TRUNCATE, DL, VT, Tmp));
    break;
...
#define CRC_CASE_EXT_BINARYOP(NAME, NODE)                                      \
  case Intrinsic::loongarch_##NAME: {                                          \
    Results.push_back(DAG.getNode(                                             \
        ISD::TRUNCATE, DL, VT,                                                 \
        DAG.getNode(RASMISD::NODE, DL, MVT::i64,                          \
...
#define CRC_CASE_EXT_UNARYOP(NAME, NODE)                                       \
  case Intrinsic::loongarch_##NAME: {                                          \
    Results.push_back(DAG.getNode(                                             \
        ISD::TRUNCATE, DL, VT,                                                 \
        DAG.getNode(RASMISD::NODE, DL, MVT::i64, Op2,                     \
                    DAG.getNode(ISD::ANY_EXTEND, DL, MVT::i64, Op3))));        \
    Results.push_back(N->getOperand(0));                                       \
    break;                                                                     \
  }
...
static SDValue performANDCombine(SDNode *N, SelectionDAG &DAG,
  ...
  msb = lsb + SMLen - 1;
  return DAG.getNode(RASMISD::BSTRPICK, DL, ValTy, NewOperand,
...
static SDValue performSRLCombine(SDNode *N, SelectionDAG &DAG,
  ...
  if (MaskIdx <= Shamt && Shamt <= MaskIdx + MaskLen - 1)
    return DAG.getNode(RASMISD::BSTRPICK, DL, ValTy,
...
Retry:
  ...
  if (N0.getOpcode() == ISD::AND &&
    ...
    return DAG.getNode(RASMISD::BSTRINS, DL, ValTy, N0.getOperand(0),
  ...
  if (N0.getOpcode() == ISD::AND &&
    ...
    return DAG.getNode(
        RASMISD::BSTRINS, DL, ValTy, N0.getOperand(0),
  ...
  if (N1.getOpcode() == ISD::SHL && N1.getOperand(0).getOpcode() == ISD::AND &&
    ...
    if (ShMask.isSubsetOf(DAG.computeKnownBits(N0).Zero)) {
      ...
      return DAG.getNode(RASMISD::BSTRINS, DL, ValTy, N0,
                         N1.getOperand(0).getOperand(0),
...
static SDValue performBITREV_WCombine(SDNode *N, SelectionDAG &DAG,
  ...
  SDValue Src = N->getOperand(0);
  if (Src.getOpcode() != RASMISD::REVB_2W)
    return SDValue();

  return DAG.getNode(RASMISD::BITREV_4B, SDLoc(N), N->getValueType(0),
                     Src.getOperand(0));
}

SDValue RASMTargetLowering::PerformDAGCombine(SDNode *N,
...
static MachineBasicBlock *insertDivByZeroTrap(MachineInstr &MI,
  ...
  // MBB:
  BuildMI(MBB, DL, TII.get(RASM::BNEZ))
  ...
  BuildMI(BreakMBB, DL, TII.get(RASM::BREAK)).addImm(7 /*BRK_DIVZERO*/);
...
MachineBasicBlock *RASMTargetLowering::EmitInstrWithCustomInserter(
...
const char *RASMTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((RASMISD::NodeType)Opcode) {
  case RASMISD::FIRST_NUMBER:
    break;
...
const MCPhysReg ArgGPRs[] = {RASM::R4,  RASM::R5, RASM::R6,
                             RASM::R7,  RASM::R8, RASM::R9,
                             RASM::R10, RASM::R11};
...
const MCPhysReg ArgFPR32s[] = {RASM::F0, RASM::F1, RASM::F2,
                               RASM::F3, RASM::F4, RASM::F5,
                               RASM::F6, RASM::F7};
// FPR32 and FPR64 alias each other.
const MCPhysReg ArgFPR64s[] = {
    RASM::F0_64, RASM::F1_64, RASM::F2_64, RASM::F3_64,
    RASM::F4_64, RASM::F5_64, RASM::F6_64, RASM::F7_64};

// Pass a 2*GRLen argument that has been split into two GRLen values through
// registers or the stack as necessary.
static bool CC_RASMAssign2GRLen(unsigned GRLen, CCState &State,
...
// Implements the RASM calling convention. Returns true upon failure.
static bool CC_RASM(const DataLayout &DL, RASMABI::ABI ABI,
  ...
  // If this is a variadic argument, the RASM calling convention requires
  ...
  if (ValVT.isScalarInteger() && ArgFlags.isSplitEnd() &&
    ...
    return CC_RASMAssign2GRLen(GRLen, State, VA, AF, ValNo, ValVT, LocVT,
                                    ArgFlags);
  }
...
void RASMTargetLowering::analyzeInputArgs(
    MachineFunction &MF, CCState &CCInfo,
    const SmallVectorImpl<ISD::InputArg> &Ins, bool IsRet,
    RASMCCAssignFn Fn) const {
  ...
  for (unsigned i = 0, e = Ins.size(); i != e; ++i) {
    ...
    RASMABI::ABI ABI =
        MF.getSubtarget<RASMSubtarget>().getTargetABI();
...
void RASMTargetLowering::analyzeOutputArgs(
    MachineFunction &MF, CCState &CCInfo,
    const SmallVectorImpl<ISD::OutputArg> &Outs, bool IsRet,
    CallLoweringInfo *CLI, RASMCCAssignFn Fn) const {
...
static SDValue convertLocVTToValVT(SelectionDAG &DAG, SDValue Val,
  ...
  case CCValAssign::BCvt:
    if (VA.getLocVT() == MVT::i64 && VA.getValVT() == MVT::f32)
      Val = DAG.getNode(RASMISD::MOVGR2FR_W_LA64, DL, MVT::f32, Val);
    else
...
static SDValue convertValVTToLocVT(SelectionDAG &DAG, SDValue Val,
  ...
  case CCValAssign::BCvt:
    if (VA.getLocVT() == MVT::i64 && VA.getValVT() == MVT::f32)
      Val = DAG.getNode(RASMISD::MOVFR2GR_S_LA64, DL, MVT::i64, Val);
    else
...
SDValue RASMTargetLowering::LowerFormalArguments(
  ...
  analyzeInputArgs(MF, CCInfo, Ins, /*IsRet=*/false, CC_RASM);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &RASM::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    auto *RASMFI = MF.getInfo<RASMMachineFunctionInfo>();
    ...
    RASMFI->setVarArgsFrameIndex(FI);
    ...
    RASMFI->setVarArgsSaveSize(VarArgsSaveSize);
  }
...
bool RASMTargetLowering::isEligibleForTailCallOptimization(
  ...
  const RASMRegisterInfo *TRI = Subtarget.getRegisterInfo();
...
SDValue
RASMTargetLowering::LowerCall(CallLoweringInfo &CLI,
  ...
  analyzeOutputArgs(MF, ArgCCInfo, Outs, /*IsRet=*/false, &CLI, CC_RASM);
  ...
  for (unsigned i = 0, j = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (!StackPtr.getNode())
        StackPtr = DAG.getCopyFromReg(Chain, DL, RASM::R3, PtrVT);
      SDValue Address =
  ...
  if (IsTailCall) {
    ...
    return DAG.getNode(RASMISD::TAIL, DL, NodeTys, Ops);
  }

  Chain = DAG.getNode(RASMISD::CALL, DL, NodeTys, Ops);
  ...
  analyzeInputArgs(MF, RetCCInfo, Ins, /*IsRet=*/true, CC_RASM);
...
bool RASMTargetLowering::CanLowerReturn(
  ...
  for (unsigned i = 0, e = Outs.size(); i != e; ++i) {
    RASMABI::ABI ABI =
        MF.getSubtarget<RASMSubtarget>().getTargetABI();
    if (CC_RASM(MF.getDataLayout(), ABI, i, Outs[i].VT, CCValAssign::Full,
...
SDValue RASMTargetLowering::LowerReturn(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
  ...
  return DAG.getNode(RASMISD::RET, DL, MVT::Other, RetOps);
}

bool RASMTargetLowering::isFPImmLegal(const APFloat &Imm, EVT VT,
...
bool RASMTargetLowering::isCheapToSpeculateCttz(Type *) const {
  return true;
}

bool RASMTargetLowering::isCheapToSpeculateCtlz(Type *) const {
  return true;
}

bool RASMTargetLowering::shouldInsertFencesForAtomic(
...
EVT RASMTargetLowering::getSetCCResultType(const DataLayout &DL,
                                                LLVMContext &Context,
                                                EVT VT) const {
  if (!VT.isVector())
...
bool RASMTargetLowering::hasAndNot(SDValue Y) const {
  // TODO: Support vectors.
  return Y.getValueType().isScalarInteger() && !isa<ConstantSDNode>(Y);
}

bool RASMTargetLowering::getTgtMemIntrinsic(IntrinsicInfo &Info,
                                                 const CallInst &I,
                                                 MachineFunction &MF,
...
RASMTargetLowering::shouldExpandAtomicRMWInIR(AtomicRMWInst *AI) const {
...
TargetLowering::AtomicExpansionKind
RASMTargetLowering::shouldExpandAtomicCmpXchgInIR(
...
Value *RASMTargetLowering::emitMaskedAtomicCmpXchgIntrinsic(
...
Value *RASMTargetLowering::emitMaskedAtomicRMWIntrinsic(
...
bool RASMTargetLowering::isFMAFasterThanFMulAndFAdd(
...
Register RASMTargetLowering::getExceptionPointerRegister(
    const Constant *PersonalityFn) const {
  return RASM::R4;
}

Register RASMTargetLowering::getExceptionSelectorRegister(
    const Constant *PersonalityFn) const {
  return RASM::R5;
}

//===----------------------------------------------------------------------===//
//                           RASM Inline Assembly Support
//===----------------------------------------------------------------------===//

RASMTargetLowering::ConstraintType
RASMTargetLowering::getConstraintType(StringRef Constraint) const {
  // RASM specific constraints in GCC: config/loongarch/constraints.md
  //
...
unsigned RASMTargetLowering::getInlineAsmMemConstraint(
...
RASMTargetLowering::getRegForInlineAsmConstraint(
    const TargetRegisterInfo *TRI, StringRef Constraint, MVT VT) const {
  // First, see if this is a constraint that directly corresponds to a RASM
  // register class.
  if (Constraint.size() == 1) {
    switch (Constraint[0]) {
    case 'r':
      ...
      if (VT.isVector())
        break;
      return std::make_pair(0U, &RASM::GPRRegClass);
    case 'f':
      if (Subtarget.hasBasicF() && VT == MVT::f32)
        return std::make_pair(0U, &RASM::FPR32RegClass);
      if (Subtarget.hasBasicD() && VT == MVT::f64)
        return std::make_pair(0U, &RASM::FPR64RegClass);
      break;
    default:
      break;
    }
  ...
  // AFAIK, the not yet upstreamed `rustc` for RASM will always use
  ...
  if (Constraint.startswith("{$r") || Constraint.startswith("{$f")) {
    ...
    if (IsFP) {
      ...
      if (RASM::F0 <= RegNo && RegNo <= RASM::F31) {
        if (Subtarget.hasBasicD() && (VT == MVT::f64 || VT == MVT::Other)) {
          unsigned DReg = RegNo - RASM::F0 + RASM::F0_64;
          return std::make_pair(DReg, &RASM::FPR64RegClass);
        }
      }
    }
    return R;
...
void RASMTargetLowering::LowerAsmOperandForConstraint(
...
#include "RASMGenAsmMatcher.inc"

Register
RASMTargetLowering::getRegisterByName(const char *RegName, LLT VT,
