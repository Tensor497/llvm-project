//===- RASMMatInt.cpp - Immediate materialisation ---------*- C++ -*--===//
//
...
#include "RASMMatInt.h"
#include "MCTargetDesc/RASMMCTargetDesc.h"
#include "llvm/Support/MathExtras.h"

using namespace llvm;

RASMMatInt::InstSeq RASMMatInt::generateInstSeq(int64_t Val) {
  // Val:
  // |            hi32              |              lo32            |
  // +-----------+------------------+------------------+-----------+
  ...
  if (Highest12 != 0 && SignExtend64<52>(Val) == 0) {
    Insts.push_back(Inst(RASM::LU52I_D, SignExtend64<12>(Highest12)));
    return Insts;
  }

  if (Hi20 == 0)
    Insts.push_back(Inst(RASM::ORI, Lo12));
  else if (SignExtend32<1>(Lo12 >> 11) == SignExtend32<20>(Hi20))
    Insts.push_back(Inst(RASM::ADDI_W, SignExtend64<12>(Lo12)));
  else {
    Insts.push_back(Inst(RASM::LU12I_W, SignExtend64<20>(Hi20)));
    if (Lo12 != 0)
      Insts.push_back(Inst(RASM::ORI, Lo12));
  }

  if (SignExtend32<1>(Hi20 >> 19) != SignExtend32<20>(Higher20))
    Insts.push_back(Inst(RASM::LU32I_D, SignExtend64<20>(Higher20)));
