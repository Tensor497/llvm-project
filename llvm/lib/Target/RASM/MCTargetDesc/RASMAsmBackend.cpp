//===-- RASMAsmBackend.cpp - RASM Assembler Backend -*- C++ -*---===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the RASMAsmBackend class.
//
//===----------------------------------------------------------------------===//
...
RASMAsmBackend::getFixupKind(StringRef Name) const {
  if (STI.getTargetTriple().isOSBinFormatELF()) {
    auto Type = llvm::StringSwitch<unsigned>(Name)
#define ELF_RELOC(X, Y) .Case(#X, Y)
#include "llvm/BinaryFormat/ELFRelocs/RASM.def"
#undef ELF_RELOC
...
RASMAsmBackend::getFixupKindInfo(MCFixupKind Kind) const {
  ...
  static_assert((std::size(Infos)) == RASM::NumTargetFixupKinds,
...
void RASMAsmBackend::applyFixup(const MCAssembler &Asm,
                                     const MCFixup &Fixup,
...
bool RASMAsmBackend::shouldForceRelocation(const MCAssembler &Asm,
                                                const MCFixup &Fixup,
                                                const MCValue &Target) {
...
bool RASMAsmBackend::writeNopData(raw_ostream &OS, uint64_t Count,
                                       const MCSubtargetInfo *STI) const {
  // Check for byte count not multiple of instruction word size
  if (Count % 4 != 0)
    return false;

  // The nop on RASM is andi r0, r0, 0.
  for (; Count >= 4; Count -= 4)
...
RASMAsmBackend::createObjectTargetWriter() const {
  return createRASMELFObjectWriter(OSABI, Is64Bit);
}

MCAsmBackend *llvm::createRASMAsmBackend(const Target &T,
                                              const MCSubtargetInfo &STI,
                                              const MCRegisterInfo &MRI,
                                              const MCTargetOptions &Options) {
  const Triple &TT = STI.getTargetTriple();
  uint8_t OSABI = MCELFObjectTargetWriter::getOSABI(TT.getOS());
  return new RASMAsmBackend(STI, OSABI, TT.isArch64Bit());
}
