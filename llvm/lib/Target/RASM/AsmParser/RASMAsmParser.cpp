// RASMAsmParser.cpp - Parse RASM assembly to MCInst instructions -=//
//
...
namespace {
class RASMAsmParser : public MCTargetAsmParser {
  SMLoc getLoc() const { return getParser().getTok().getLoc(); }
  bool is64Bit() const { return getSTI().hasFeature(RASM::Feature64Bit); }

  struct Inst {
    unsigned Opc;
    RASMMCExpr::VariantKind VK;
    Inst(unsigned Opc,
         RASMMCExpr::VariantKind VK = RASMMCExpr::VK_RASM_None)
        : Opc(Opc), VK(VK) {}
  };
...
#undef GET_OPERAND_DIAGNOSTIC_TYPES
  ...
  RASMAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
// Instances of this class represent a parsed RASM machine instruction.
class RASMOperand : public MCParsedAsmOperand {
  enum class KindTy {
    Token,
    Register,
    Immediate,
...
public:
  RASMOperand(KindTy K) : MCParsedAsmOperand(), Kind(K) {}
  ...
  static bool evaluateConstantImm(const MCExpr *Expr, int64_t &Imm,
                                  RASMMCExpr::VariantKind &VK) {
    if (auto *LE = dyn_cast<RASMMCExpr>(Expr)) {
      VK = LE->getKind();
      return false;
    }
  ...
  bool isBareSymbol() const {
    ...
    if (!isImm() || evaluateConstantImm(getImm(), Imm, VK))
      return false;
    return RASMAsmParser::classifySymbolRef(getImm(), VK) &&
           VK == RASMMCExpr::VK_RASM_None;
  }
  ...
  bool isSImm12addlike() const {
    ...
    RASMMCExpr::VariantKind VK = RASMMCExpr::VK_RASM_None;
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    bool IsValidKind = VK == RASMMCExpr::VK_RASM_None ||
                       VK == RASMMCExpr::VK_RASM_PCALA_LO12 ||
                       VK == RASMMCExpr::VK_RASM_GOT_PC_LO12 ||
                       VK == RASMMCExpr::VK_RASM_TLS_IE_PC_LO12;
    return IsConstantImm
               ? isInt<12>(Imm) && IsValidKind
               : RASMAsmParser::classifySymbolRef(getImm(), VK) &&
                     IsValidKind;
  }
  ...
  bool isSImm12lu52id() const {
    ...
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    bool IsValidKind = VK == RASMMCExpr::VK_RASM_None ||
                       VK == RASMMCExpr::VK_RASM_ABS64_HI12 ||
                       VK == RASMMCExpr::VK_RASM_PCALA64_HI12 ||
                       VK == RASMMCExpr::VK_RASM_GOT64_HI12 ||
                       VK == RASMMCExpr::VK_RASM_GOT64_PC_HI12 ||
                       VK == RASMMCExpr::VK_RASM_TLS_LE64_HI12 ||
                       VK == RASMMCExpr::VK_RASM_TLS_IE64_HI12 ||
                       VK == RASMMCExpr::VK_RASM_TLS_IE64_PC_HI12;
    return IsConstantImm
               ? isInt<12>(Imm) && IsValidKind
               : RASMAsmParser::classifySymbolRef(getImm(), VK) &&
                     IsValidKind;
  ...
  bool isUImm12ori() const {
    ...
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    bool IsValidKind = VK == RASMMCExpr::VK_RASM_None ||
                       VK == RASMMCExpr::VK_RASM_ABS_LO12 ||
                       VK == RASMMCExpr::VK_RASM_PCALA_LO12 ||
                       VK == RASMMCExpr::VK_RASM_GOT_LO12 ||
                       VK == RASMMCExpr::VK_RASM_GOT_PC_LO12 ||
                       VK == RASMMCExpr::VK_RASM_TLS_LE_LO12 ||
                       VK == RASMMCExpr::VK_RASM_TLS_IE_LO12 ||
                       VK == RASMMCExpr::VK_RASM_TLS_IE_PC_LO12;
    return IsConstantImm
  ...
  bool isSImm20pcalau12i() const {
    ...
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    bool IsValidKind = VK == RASMMCExpr::VK_RASM_None ||
                       VK == RASMMCExpr::VK_RASM_PCALA_HI20 ||
                       VK == RASMMCExpr::VK_RASM_GOT_PC_HI20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_IE_PC_HI20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_LD_PC_HI20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_GD_PC_HI20;
    return IsConstantImm
  ...
  bool isSImm20lu12iw() const {
    ...
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    bool IsValidKind = VK == RASMMCExpr::VK_RASM_None ||
                       VK == RASMMCExpr::VK_RASM_ABS_HI20 ||
                       VK == RASMMCExpr::VK_RASM_GOT_HI20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_GD_HI20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_LD_HI20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_IE_HI20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_LE_HI20;
    return IsConstantImm
               ? isInt<20>(Imm) && IsValidKind
               : RASMAsmParser::classifySymbolRef(getImm(), VK) &&
                     IsValidKind;
  ...
  bool isSImm20lu32id() const {
    ...
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    bool IsValidKind = VK == RASMMCExpr::VK_RASM_None ||
                       VK == RASMMCExpr::VK_RASM_ABS64_LO20 ||
                       VK == RASMMCExpr::VK_RASM_PCALA64_LO20 ||
                       VK == RASMMCExpr::VK_RASM_GOT64_LO20 ||
                       VK == RASMMCExpr::VK_RASM_GOT64_PC_LO20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_IE64_LO20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_IE64_PC_LO20 ||
                       VK == RASMMCExpr::VK_RASM_TLS_LE64_LO20;
  ...
  bool isSImm26Operand() const {
    ...
    bool IsConstantImm = evaluateConstantImm(getImm(), Imm, VK);
    bool IsValidKind = VK == RASMMCExpr::VK_RASM_None ||
                       VK == RASMMCExpr::VK_RASM_CALL ||
                       VK == RASMMCExpr::VK_RASM_CALL_PLT ||
                       VK == RASMMCExpr::VK_RASM_B26;
    return IsConstantImm
  ...
  void print(raw_ostream &OS) const override {
    auto RegName = [](unsigned Reg) {
      if (Reg)
        return RASMInstPrinter::getRegisterName(Reg);
      else
        return "noreg";
    };
  ...
  static std::unique_ptr<RASMOperand> createToken(StringRef Str, SMLoc S) {
    auto Op = std::make_unique<RASMOperand>(KindTy::Token);
    Op->Tok = Str;
    Op->StartLoc = S;
    Op->EndLoc = S;
    return Op;
  ...
  static std::unique_ptr<RASMOperand> createReg(unsigned RegNo, SMLoc S,
                                                     SMLoc E) {
    auto Op = std::make_unique<RASMOperand>(KindTy::Register);
    Op->Reg.RegNum = RegNo;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
  ...
  static std::unique_ptr<RASMOperand> createImm(const MCExpr *Val, SMLoc S,
                                                     SMLoc E) {
    auto Op = std::make_unique<RASMOperand>(KindTy::Immediate);
    Op->Imm.Val = Val;
    Op->StartLoc = S;
    Op->EndLoc = E;
    return Op;
...
static MCRegister convertFPR32ToFPR64(MCRegister Reg) {
  assert(Reg >= RASM::F0 && Reg <= RASM::F31 && "Invalid register");
  return Reg - RASM::F0 + RASM::F0_64;
}
...
static bool matchRegisterNameHelper(MCRegister &RegNo, StringRef Name) {
  ...
  assert(!(RegNo >= RASM::F0_64 && RegNo <= RASM::F31_64));
  // The default FPR register class is based on the tablegen enum ordering.
  static_assert(RASM::F0 < RASM::F0_64,
                "FPR matching must be updated");
  if (RegNo == RASM::NoRegister)
    RegNo = MatchRegisterAltName(Name);

  return RegNo == RASM::NoRegister;
}

bool RASMAsmParser::ParseRegister(unsigned &RegNo, SMLoc &StartLoc,
                                       SMLoc &EndLoc) {
  return Error(getLoc(), "invalid register number");
}

OperandMatchResultTy RASMAsmParser::tryParseRegister(unsigned &RegNo,
                                                          SMLoc &StartLoc,
                                                          SMLoc &EndLoc) {
  llvm_unreachable("Unimplemented function.");
}

bool RASMAsmParser::classifySymbolRef(const MCExpr *Expr,
                                           RASMMCExpr::VariantKind &Kind) {
  Kind = RASMMCExpr::VK_RASM_None;

  if (const RASMMCExpr *RE = dyn_cast<RASMMCExpr>(Expr)) {
    Kind = RE->getKind();
    Expr = RE->getSubExpr();
  }
  ...
  if (Expr->evaluateAsRelocatable(Res, nullptr, nullptr))
    return Res.getRefKind() == RASMMCExpr::VK_RASM_None;
  return false;
}

OperandMatchResultTy
RASMAsmParser::parseRegister(OperandVector &Operands) {
  ...
  getLexer().Lex();
  Operands.push_back(RASMOperand::createReg(RegNo, S, E));

  return MatchOperand_Success;
}

OperandMatchResultTy
RASMAsmParser::parseImmediate(OperandVector &Operands) {
  SMLoc S = getLoc();
  SMLoc E;
  const MCExpr *Res;
  ...
  Operands.push_back(RASMOperand::createImm(Res, S, E));
  return MatchOperand_Success;
}

OperandMatchResultTy
RASMAsmParser::parseOperandWithModifier(OperandVector &Operands) {
  SMLoc S = getLoc();
  SMLoc E;
  ...
  RASMMCExpr::VariantKind VK =
      RASMMCExpr::getVariantKindForName(Identifier);
  if (VK == RASMMCExpr::VK_RASM_Invalid) {
  ...
  const MCExpr *ModExpr = RASMMCExpr::create(SubExpr, VK, getContext());
  Operands.push_back(RASMOperand::createImm(ModExpr, S, E));
  return MatchOperand_Success;
}

OperandMatchResultTy
RASMAsmParser::parseSImm26Operand(OperandVector &Operands) {
  SMLoc S = getLoc();
  const MCExpr *Res;
  ...
  Res = RASMMCExpr::create(Res, RASMMCExpr::VK_RASM_CALL,
                                getContext());
  Operands.push_back(RASMOperand::createImm(Res, S, E));
  return MatchOperand_Success;
}
...
bool RASMAsmParser::parseOperand(OperandVector &Operands,
                                      StringRef Mnemonic) {
...
bool RASMAsmParser::ParseInstruction(ParseInstructionInfo &Info,
                                          StringRef Name, SMLoc NameLoc,
                                          OperandVector &Operands) {
  // First operand in MCInst is instruction mnemonic.
  Operands.push_back(RASMOperand::createToken(Name, NameLoc));
...
void RASMAsmParser::emitLAInstSeq(MCRegister DestReg, MCRegister TmpReg,
                                       const MCExpr *Symbol,
                                       SmallVectorImpl<Inst> &Insts,
                                       SMLoc IDLoc, MCStreamer &Out) {
  MCContext &Ctx = getContext();
  for (RASMAsmParser::Inst &Inst : Insts) {
    unsigned Opc = Inst.Opc;
    RASMMCExpr::VariantKind VK = Inst.VK;
    const RASMMCExpr *LE = RASMMCExpr::create(Symbol, VK, Ctx);
    switch (Opc) {
    default:
    ...
    case RASM::ADDI_D:
      Out.emitInstruction(
          MCInstBuilder(Opc)
              .addReg(TmpReg)
              .addReg(DestReg == TmpReg ? TmpReg : RASM::R0)
              .addExpr(LE),
          getSTI());
      break;
    case RASM::ADD_D:
...
void RASMAsmParser::emitLoadAddressAbs(MCInst &Inst, SMLoc IDLoc,
                                            MCStreamer &Out) {
  // la.abs $rd, sym
  // expands to:
  ...
  const MCExpr *Symbol = Inst.getOpcode() == RASM::PseudoLA_ABS
                             ? Inst.getOperand(1).getExpr()
                             : Inst.getOperand(2).getExpr();
  InstSeq Insts;

  Insts.push_back(RASMAsmParser::Inst(
      RASM::LU12I_W, RASMMCExpr::VK_RASM_ABS_HI20));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::ORI, RASMMCExpr::VK_RASM_ABS_LO12));

  if (is64Bit()) {
    Insts.push_back(RASMAsmParser::Inst(
        RASM::LU32I_D, RASMMCExpr::VK_RASM_ABS64_LO20));
    Insts.push_back(RASMAsmParser::Inst(
        RASM::LU52I_D, RASMMCExpr::VK_RASM_ABS64_HI12));
  }

  emitLAInstSeq(DestReg, DestReg, Symbol, Insts, IDLoc, Out);
}

void RASMAsmParser::emitLoadAddressPcrel(MCInst &Inst, SMLoc IDLoc,
                                              MCStreamer &Out) {
  // la.pcrel $rd, sym
  // expands to:
  ...
  InstSeq Insts;
  unsigned ADDI = is64Bit() ? RASM::ADDI_D : RASM::ADDI_W;

  Insts.push_back(RASMAsmParser::Inst(
      RASM::PCALAU12I, RASMMCExpr::VK_RASM_PCALA_HI20));
  Insts.push_back(
      RASMAsmParser::Inst(ADDI, RASMMCExpr::VK_RASM_PCALA_LO12));

  emitLAInstSeq(DestReg, DestReg, Symbol, Insts, IDLoc, Out);
}

void RASMAsmParser::emitLoadAddressPcrelLarge(MCInst &Inst, SMLoc IDLoc,
                                                   MCStreamer &Out) {
  // la.pcrel $rd, $rj, sym
  // expands to:
  //   pcalau12i $rd, %pc_hi20(sym)
  ...
  Insts.push_back(RASMAsmParser::Inst(
      RASM::PCALAU12I, RASMMCExpr::VK_RASM_PCALA_HI20));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::ADDI_D, RASMMCExpr::VK_RASM_PCALA_LO12));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::LU32I_D, RASMMCExpr::VK_RASM_PCALA64_LO20));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::LU52I_D, RASMMCExpr::VK_RASM_PCALA64_HI12));
  Insts.push_back(RASMAsmParser::Inst(RASM::ADD_D));

  emitLAInstSeq(DestReg, TmpReg, Symbol, Insts, IDLoc, Out);
}

void RASMAsmParser::emitLoadAddressGot(MCInst &Inst, SMLoc IDLoc,
                                            MCStreamer &Out) {
  // la.got $rd, sym
  // expands to:
  ...
  const MCExpr *Symbol = Inst.getOperand(1).getExpr();
  InstSeq Insts;
  unsigned LD = is64Bit() ? RASM::LD_D : RASM::LD_W;

  Insts.push_back(RASMAsmParser::Inst(
      RASM::PCALAU12I, RASMMCExpr::VK_RASM_GOT_PC_HI20));
  Insts.push_back(
      RASMAsmParser::Inst(LD, RASMMCExpr::VK_RASM_GOT_PC_LO12));

  emitLAInstSeq(DestReg, DestReg, Symbol, Insts, IDLoc, Out);
}

void RASMAsmParser::emitLoadAddressGotLarge(MCInst &Inst, SMLoc IDLoc,
                                                 MCStreamer &Out) {
  // la.got $rd, $rj, sym
  // expands to:
  //   pcalau12i $rd, %got_pc_hi20(sym)
  ...
  Insts.push_back(RASMAsmParser::Inst(
      RASM::PCALAU12I, RASMMCExpr::VK_RASM_GOT_PC_HI20));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::ADDI_D, RASMMCExpr::VK_RASM_GOT_PC_LO12));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::LU32I_D, RASMMCExpr::VK_RASM_GOT64_PC_LO20));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::LU52I_D, RASMMCExpr::VK_RASM_GOT64_PC_HI12));
  Insts.push_back(RASMAsmParser::Inst(RASM::LDX_D));

  emitLAInstSeq(DestReg, TmpReg, Symbol, Insts, IDLoc, Out);
}

void RASMAsmParser::emitLoadAddressTLSLE(MCInst &Inst, SMLoc IDLoc,
                                              MCStreamer &Out) {
  // la.tls.le $rd, sym
  // expands to:
  ...
  Insts.push_back(RASMAsmParser::Inst(
      RASM::LU12I_W, RASMMCExpr::VK_RASM_TLS_LE_HI20));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::ORI, RASMMCExpr::VK_RASM_TLS_LE_LO12));

  emitLAInstSeq(DestReg, DestReg, Symbol, Insts, IDLoc, Out);
}

void RASMAsmParser::emitLoadAddressTLSIE(MCInst &Inst, SMLoc IDLoc,
                                              MCStreamer &Out) {
  // la.tls.ie $rd, sym
  // expands to:
  ...
  Insts.push_back(RASMAsmParser::Inst(
      RASM::PCALAU12I, RASMMCExpr::VK_RASM_TLS_IE_PC_HI20));
  Insts.push_back(RASMAsmParser::Inst(
      LD, RASMMCExpr::VK_RASM_TLS_IE_PC_LO12));

  emitLAInstSeq(DestReg, DestReg, Symbol, Insts, IDLoc, Out);
}

void RASMAsmParser::emitLoadAddressTLSIELarge(MCInst &Inst, SMLoc IDLoc,
                                                   MCStreamer &Out) {
  // la.tls.ie $rd, $rj, sym
  // expands to:
  //   pcalau12i $rd, %ie_pc_hi20(sym)
  ...
  Insts.push_back(RASMAsmParser::Inst(
      RASM::PCALAU12I, RASMMCExpr::VK_RASM_TLS_IE_PC_HI20));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::ADDI_D, RASMMCExpr::VK_RASM_TLS_IE_PC_LO12));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::LU32I_D, RASMMCExpr::VK_RASM_TLS_IE64_PC_LO20));
  Insts.push_back(RASMAsmParser::Inst(
      RASM::LU52I_D, RASMMCExpr::VK_RASM_TLS_IE64_PC_HI12));
  Insts.push_back(RASMAsmParser::Inst(RASM::LDX_D));

  emitLAInstSeq(DestReg, TmpReg, Symbol, Insts, IDLoc, Out);
}

void RASMAsmParser::emitLoadAddressTLSLD(MCInst &Inst, SMLoc IDLoc,
                                              MCStreamer &Out) {
  // la.tls.ld $rd, sym
  // expands to:
  ...
  Insts.push_back(RASMAsmParser::Inst(
      RASM::PCALAU12I, RASMMCExpr::VK_RASM_TLS_LD_PC_HI20));
  Insts.push_back(RASMAsmParser::Inst(
      ADDI, RASMMCExpr::VK_RASM_GOT_PC_LO12));

  emitLAInstSeq(DestReg, DestReg, Symbol, Insts, IDLoc, Out);
}

void RASMAsmParser::emitLoadAddressTLSLDLarge(MCInst &Inst, SMLoc IDLoc,
                                                   MCStreamer &Out) {
  // la.tls.ld $rd, $rj, sym
  // expands to:
  //   pcalau12i $rd, %ld_pc_hi20(sym)
...
void RASMAsmParser::emitLoadAddressTLSGD(MCInst &Inst, SMLoc IDLoc,
                                              MCStreamer &Out) {
  // la.tls.gd $rd, sym
  // expands to:
  ...
  Insts.push_back(RASMAsmParser::Inst(
      RASM::PCALAU12I, RASMMCExpr::VK_RASM_TLS_GD_PC_HI20));
  Insts.push_back(RASMAsmParser::Inst(
      ADDI, RASMMCExpr::VK_RASM_GOT_PC_LO12));
...
void RASMAsmParser::emitLoadAddressTLSGDLarge(MCInst &Inst, SMLoc IDLoc,
                                                   MCStreamer &Out) {
  // la.tls.gd $rd, $rj, sym
  // expands to:
  //   pcalau12i $rd, %gd_pc_hi20(sym)
...
void RASMAsmParser::emitLoadImm(MCInst &Inst, SMLoc IDLoc,
                                     MCStreamer &Out) {
  MCRegister DestReg = Inst.getOperand(0).getReg();
  int64_t Imm = Inst.getOperand(1).getImm();
  MCRegister SrcReg = RASM::R0;

  if (Inst.getOpcode() == RASM::PseudoLI_W)
...
bool RASMAsmParser::processInstruction(MCInst &Inst, SMLoc IDLoc,
                                            OperandVector &Operands,
                                            MCStreamer &Out) {
  Inst.setLoc(IDLoc);
...
unsigned RASMAsmParser::checkTargetMatchPredicate(MCInst &Inst) {
  unsigned Opc = Inst.getOpcode();
  switch (Opc) {
  default:
    if (Opc >= RASM::AMADD_D && Opc <= RASM::AMXOR_W) {
  ...
  case RASM::CSRXCHG: {
    unsigned Rj = Inst.getOperand(2).getReg();
    if (Rj == RASM::R0 || Rj == RASM::R1)
      return Match_RequiresOpnd2NotR0R1;
    return Match_Success;
  }
  case RASM::BSTRINS_W:
  case RASM::BSTRINS_D:
  case RASM::BSTRPICK_W:
  case RASM::BSTRPICK_D: {
    unsigned Opc = Inst.getOpcode();
    const signed Msb =
        (Opc == RASM::BSTRINS_W || Opc == RASM::BSTRINS_D)
...
unsigned
RASMAsmParser::validateTargetOperandClass(MCParsedAsmOperand &AsmOp,
                                               unsigned Kind) {
  RASMOperand &Op = static_cast<RASMOperand &>(AsmOp);
  if (!Op.isReg())
  ...
  if (RASMMCRegisterClasses[RASM::FPR32RegClassID].contains(Reg) &&
      Kind == MCK_FPR64) {
...
bool RASMAsmParser::generateImmOutOfRangeError(
    OperandVector &Operands, uint64_t ErrorInfo, int64_t Lower, int64_t Upper,
    Twine Msg = "immediate must be an integer in the range") {
  SMLoc ErrorLoc = ((RASMOperand &)*Operands[ErrorInfo]).getStartLoc();
  return Error(ErrorLoc, Msg + " [" + Twine(Lower) + ", " + Twine(Upper) + "]");
}

bool RASMAsmParser::MatchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
                                                 OperandVector &Operands,
                                                 MCStreamer &Out,
                                                 uint64_t &ErrorInfo,
  ...
  case Match_MnemonicFail: {
    ...
    std::string Suggestion = RASMMnemonicSpellCheck(
        ((RASMOperand &)*Operands[0]).getToken(), FBS, 0);
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0ULL) {
      ...
      ErrorLoc = ((RASMOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IDLoc;
    }
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeRASMAsmParser() {
  RegisterMCAsmParser<RASMAsmParser> X(getTheRASM32Target());
  RegisterMCAsmParser<RASMAsmParser> Y(getTheRASM64Target());
}
