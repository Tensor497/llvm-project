//===-- RASMFrameLowering.cpp - RASM Frame Information -*- C++ -*-==//
//
...
//===----------------------------------------------------------------------===//
//
// This file contains the RASM implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
bool RASMFrameLowering::hasFP(const MachineFunction &MF) const {
...
bool RASMFrameLowering::hasBP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
void RASMFrameLowering::adjustReg(MachineBasicBlock &MBB,
  ...
  unsigned Opc = IsLA64 ? RASM::ADD_D : RASM::ADD_W;
  if (Val < 0) {
    Val = -Val;
    Opc = IsLA64 ? RASM::SUB_D : RASM::SUB_W;
  }
...
void RASMFrameLowering::determineFrameLayout(MachineFunction &MF) const {
...
static uint64_t estimateFunctionSizeInBytes(const RASMInstrInfo *TII,
...
static bool needScavSlotForCFR(MachineFunction &MF) {
  ...
  for (auto &MBB : MF)
    for (auto &MI : MBB)
      if (MI.getOpcode() == RASM::PseudoST_CFR)
        return true;
  return false;
}

void RASMFrameLowering::processFunctionBeforeFrameFinalized(
    MachineFunction &MF, RegScavenger *RS) const {
  const RASMRegisterInfo *RI = STI.getRegisterInfo();
...
void RASMFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  Register SPReg = RASM::R3;
  Register FPReg = RASM::R22;
  ...
  if (hasFP(MF)) {
    adjustReg(MBB, MBBI, DL, FPReg, SPReg,
              StackSize - RASMFI->getVarArgsSaveSize(),
              MachineInstr::FrameSetup);

    // Emit ".cfi_def_cfa $fp, RASMFI->getVarArgsSaveSize()"
    unsigned CFIIndex = MF.addFrameInst(
        MCCFIInstruction::cfiDefCfa(nullptr, RI->getDwarfRegNum(FPReg, true),
                                    RASMFI->getVarArgsSaveSize()));
    BuildMI(MBB, MBBI, DL, TII->get(TargetOpcode::CFI_INSTRUCTION))
        .addCFIIndex(CFIIndex)
        .setMIFlag(MachineInstr::FrameSetup);
  }
  ...
  if (FirstSPAdjustAmount && SecondSPAdjustAmount) {
    if (hasFP(MF)) {
      ...
      for (int Val = SecondSPAdjustAmount; Val > 0; Val -= 2048)
        BuildMI(MBB, MBBI, DL,
                TII->get(IsLA64 ? RASM::ADDI_D : RASM::ADDI_W), SPReg)
            .addReg(SPReg)
  ...
  if (hasFP(MF)) {
    ...
    if (RI->hasStackRealignment(MF)) {
      ...
      BuildMI(MBB, MBBI, DL,
              TII->get(IsLA64 ? RASM::SRLI_D : RASM::SRLI_W), VR)
          .addReg(SPReg)
          .addImm(ShiftAmount)
          .setMIFlag(MachineInstr::FrameSetup);
      BuildMI(MBB, MBBI, DL,
              TII->get(IsLA64 ? RASM::SLLI_D : RASM::SLLI_W), SPReg)
          .addReg(VR)
          .addImm(ShiftAmount)
          .setMIFlag(MachineInstr::FrameSetup);
      ...
      if (hasBP(MF)) {
        // move BP, $sp
        BuildMI(MBB, MBBI, DL, TII->get(RASM::OR),
                RASMABI::getBPReg())
            .addReg(SPReg)
            .addReg(RASM::R0)
            .setMIFlag(MachineInstr::FrameSetup);
...
void RASMFrameLowering::emitEpilogue(MachineFunction &MF,
                                          MachineBasicBlock &MBB) const {
  const RASMRegisterInfo *RI = STI.getRegisterInfo();
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *RASMFI = MF.getInfo<RASMMachineFunctionInfo>();
  ...
  if (RI->hasStackRealignment(MF) || MFI.hasVarSizedObjects()) {
    ...
    adjustReg(MBB, LastFrameDestroy, DL, SPReg, RASM::R22,
              -StackSize + RASMFI->getVarArgsSaveSize(),
...
//   addi.d  $sp, $sp,   -16
uint64_t
RASMFrameLowering::getFirstSPAdjustAmount(const MachineFunction &MF,
...
void RASMFrameLowering::determineCalleeSaves(MachineFunction &MF,
                                                  BitVector &SavedRegs,
  ...
  // pointer.
  if (hasFP(MF)) {
    SavedRegs.set(RASM::R1);
    SavedRegs.set(RASM::R22);
  }
  // Mark BP as used if function has dedicated base pointer.
  if (hasBP(MF))
    SavedRegs.set(RASMABI::getBPReg());
}
...
bool RASMFrameLowering::hasReservedCallFrame(
...
RASMFrameLowering::eliminateCallFramePseudoInstr(
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      if (MI->getOpcode() == RASM::ADJCALLSTACKDOWN)
        Amount = -Amount;
...
bool RASMFrameLowering::spillCalleeSavedRegisters(
  ...
  for (auto &CS : CSI) {
    ...
    // RASMTargetLowering::lowerRETURNADDR, don't set kill flag.
    bool IsKill =
        !(Reg == RASM::R1 && MF->getFrameInfo().isReturnAddressTaken());
...
StackOffset RASMFrameLowering::getFrameIndexReference(
  ...
  if (FI >= MinCSFI && FI <= MaxCSFI) {
    ...
    // after realignment.
    FrameReg = hasBP(MF) ? RASMABI::getBPReg() : RASM::R3;
    Offset += StackOffset::getFixed(StackSize);
  } else {
    FrameReg = RI->getFrameRegister(MF);
    if (hasFP(MF))
      Offset += StackOffset::getFixed(RASMFI->getVarArgsSaveSize());
    else
      Offset += StackOffset::getFixed(StackSize);
  }
